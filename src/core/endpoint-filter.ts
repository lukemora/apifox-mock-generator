...
export function filterEndpoints(endpoints: ApiEndpoint[], filter?: ApiFilter): ApiEndpoint[] {
  ...
  return endpoints.filter(endpoint => {
    ...
    // 3. 检查 excludeDeprecated（排除废弃接口，写死为 true）
    ...
    if (filter.scope?.includedByTags && filter.scope.includedByTags.length > 0) {
      // 转换配置中的状态值（支持中文）
      const includedStatuses = filter.scope.includedByTags.map(translateStatus);
    ...
    if (filter.scope?.excludedByTags && filter.scope.excludedByTags.length > 0) {
      ...
      if (endpoint.status && excludedStatuses.includes(endpoint.status)) {
        logger.debug(`[Filter] 接口状态匹配 excludedByTags，已排除: ${endpoint.status}`);
        return false;
      }
    }
