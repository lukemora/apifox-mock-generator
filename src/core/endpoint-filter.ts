 ...
 * 根据筛选配置过滤 API 端点
 */
export function filterEndpoints(endpoints: ApiEndpoint[], filter?: ApiFilter): ApiEndpoint[] {
  if (!filter) {
    return endpoints
  }
  ...
  return endpoints.filter(endpoint => {
    ...
    if (filter.includePaths && filter.includePaths.length > 0) {
      const matched = filter.includePaths.some(pattern => matchPath(endpoint.path, pattern))
      if (!matched) {
    ...
    if (filter.excludePaths && filter.excludePaths.length > 0) {
      const matched = filter.excludePaths.some(pattern => matchPath(endpoint.path, pattern))
      if (matched) {
    ...
    if (filter.includeOperationIds && filter.includeOperationIds.length > 0) {
      if (!endpoint.operationId || !filter.includeOperationIds.includes(endpoint.operationId)) {
        logger.debug(`[Filter] operationId 不匹配 includeOperationIds: ${endpoint.operationId}`)
        return false
      }
    }
    ...
    if (filter.excludeOperationIds && filter.excludeOperationIds.length > 0) {
      if (endpoint.operationId && filter.excludeOperationIds.includes(endpoint.operationId)) {
        logger.debug(`[Filter] operationId 匹配 excludeOperationIds，已排除: ${endpoint.operationId}`)
        return false
      }
    }
